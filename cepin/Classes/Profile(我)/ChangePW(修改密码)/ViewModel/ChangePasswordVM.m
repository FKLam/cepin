...
@end
@implementation ChangePasswordVM
- (instancetype)init
{
    self = [super init];
    if (self) {
...
{
    BOOL canSubmit = NO;
    if ( 0 < [self.oldPassword length] && 0 < [self.passWord length] && 0 < [self.entenPassword length] )
        canSubmit = YES;
    return canSubmit;
}
-(void)changePassword
{
    ...
    if (!self.oldPassword || [self.oldPassword isEqualToString:@""]) {
        [self showShordMessage:@"原始密码不能为空"];
        return;
    }
    if (!self.passWord || [self.passWord isEqualToString:@""]) {
        [self showShordMessage:@"请输入新密码"];
        return;
    }
    ...
    if ( 0 < [password length] && ![password isEqualToString:self.oldPassword]) {
        [self showShordMessage:@"原始密码不正确"];
        return;
    }
    if (![self.passWord isEqualToString:self.entenPassword]) {
        [self showShordMessage:@"两次输入的密码不相同，请重新输入"];
        return;
    }
    ...
    RACSignal *signal = [[RTNetworking shareInstance]changeUserPassWordWithUserId:userId tokenId:tokenId password:self.passWord oldPassword:self.oldPassword];
