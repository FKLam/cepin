...
#import "CPCommon.h"
@interface CPPraciticeLeaderCell ()
@property (nonatomic, strong) UIView *blackBackgroundView;
@property (nonatomic, strong) UIView *whiteBackgroundView;
@property (nonatomic, strong) UILabel *positionLabel;
@property (nonatomic, strong) UILabel *companyLabel;
@property (nonatomic, strong) UILabel *timeLabel;
@property (nonatomic, strong) StudentLeadersDataModel *studentLeader;
@end
@implementation CPPraciticeLeaderCell
...
{
    static NSString *reuseIdentifier = @"leader";
    CPPraciticeLeaderCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier];
    if ( !cell )
    {
        cell = [[CPPraciticeLeaderCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier];
...
- (void)configCellWithStudentLeader:(StudentLeadersDataModel *)studentLeader
{
    if ( !studentLeader )
        return;
    _studentLeader = studentLeader;
    NSString *startime =  [NSDate cepinYMDFromString:_studentLeader.StartDate];
    NSString *endTime = [NSDate cepinYMDFromString:_studentLeader.EndDate];
    if (!endTime || [endTime isEqualToString:@""]) {
        endTime = @"至今";
    }
    NSString *time = [NSString stringWithFormat:@"%@-%@",startime ,endTime];
    [self.timeLabel setText:time];
    [self.positionLabel setText:[NSString stringWithFormat:@"%@  |  %@", _studentLeader.Name, _studentLeader.Level]];
}
