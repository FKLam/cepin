//
//  CPSuccessDescripteController.m
//  cepin
//
...
#import "CPTipsView.h"
#import "CPCommon.h"
@interface CPSuccessDescripteController ()<UITextViewDelegate, CPTipsViewDelegate>
@property(nonatomic,strong)UITextView *describeText;
@property(nonatomic,strong)UILabel *titleLabel;
@property (nonatomic, strong) CPTipsView *tipsView;
@property (nonatomic, strong) NSString *successDescripte;
@end
@implementation CPSuccessDescripteController
- (instancetype)initWithEduModel:(ProjectListDataModel *)model
{
    ...
    {
        self.model = model;
        self.successDescripte = model.Description;
    }
    return self;
}
- (void)viewDidLoad
{
    ...
    [[self addNavicationObjectWithType:NavcationBarObjectTypeConfirm] handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
        ...
        if (self.describeText.text.length > 500) {
            [self shomMessageTips:@"成果描述不能超过500字符"];
            return ;
        }
        if ( [self.successDescripte length] > 0 )
        {
            if ( ![self checkWithString:self.successDescripte] )
            {
                [self shomMessageTips:@"请输入成果描述"];
                return;
            }
        }
        else
        ...
        self.model.Description = self.successDescripte;
    ...
    [self.describeText setContentInset:UIEdgeInsetsMake(0.0, -4.0, 0, 0)];
    [back addSubview:self.describeText];
    self.describeText.text = self.successDescripte;
    [[self.describeText rac_textSignal] subscribeNext:^(NSString *text) {
        self.successDescripte = text;
    }];
    ...
    [[self.describeText rac_textSignal] subscribeNext:^(NSString *text) {
        if (text) {
            ...
            if( len < 0 ){
                ...
                self.successDescripte = [text substringToIndex:500];
                return ;
            }
