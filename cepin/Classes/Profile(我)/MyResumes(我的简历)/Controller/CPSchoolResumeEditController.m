//
//  CPSchoolResumeEditController.m
//  cepin
//
...
@interface CPSchoolResumeEditController ()<FullResumeMenuViewDelegate,UIAlertViewDelegate, UIImagePickerControllerDelegate,VPImageCropperDelegate,UINavigationControllerDelegate, UITextFieldDelegate, CPTipsViewDelegate, CPResumeAttachInformationViewDelegate, SDPhotoBrowserDelegate>
...
@property (nonatomic, strong) CPResumePracticeView *practiceView;
@property (nonatomic, strong) CPResumeTrainExperienceView *trainExperienceView;
...
@end
@implementation CPSchoolResumeEditController
...
- (void)viewDidLoad {
    ...
    [RACObserve(self.viewModel, stateCode) subscribeNext:^(id stateCode){
        ...
        if(stateCode && [self requestStateWithStateCode:stateCode] == HUDCodeSucess)
        {
            ...
            [self.selfDescribeView configWithResume:self.viewModel.resumeNameModel title:nil hideMustWriteFlag:NO];
            [self.resumeWorkExperienceView configWithResume:self.viewModel.resumeNameModel title:@"实习经历" hideMustWriteFlag:YES];
            [self.projectExperienceView configWithResume:self.viewModel.resumeNameModel];
            [self.practiceView configWithResume:self.viewModel.resumeNameModel];
            [self.trainExperienceView configWithResume:self.viewModel.resumeNameModel];
...
- (void)checkDeliveryButton
{
    ...
    if ( !self.viewModel.resumeNameModel.UserRemark || 0 == [self.viewModel.resumeNameModel.UserRemark length] )
    {
        hideDeliveryButton = YES;
        [self.deliveryButton setHidden:hideDeliveryButton];
        return;
...
- (CGFloat)contentHeight
{
    ...
    tempHeight =  ResumeEditFixd_Height + [CPResumeEditReformer informationHeight:self.viewModel.resumeNameModel] + [CPResumeEditReformer expectWorkHeight:self.viewModel.resumeNameModel] + [CPResumeEditReformer educationTotalHeight:self.viewModel.resumeNameModel] + [CPResumeEditReformer selfDescribeTotalHeight:self.viewModel.resumeNameModel] + ( 40 + 144 + 40 ) / CP_GLOBALSCALE + 64.0;
    if ( self.moreButton.isSelected )
    {
        tempHeight += [CPResumeEditReformer projectExperienceTotalHeight:self.viewModel.resumeNameModel] + [CPResumeEditReformer workExperienceTotalHeight:self.viewModel.resumeNameModel] + [CPResumeEditReformer aboutSkillHeight:self.viewModel.resumeNameModel] + [CPResumeEditReformer practiceTotalHeight:self.viewModel.resumeNameModel] + [CPResumeEditReformer trainTotalHeight:self.viewModel.resumeNameModel] + [CPResumeEditReformer resumeEditAdditionHeight:self.viewModel.resumeNameModel];
        if ( self.examFinishDict )
        {
...
- (void)resetFrame
{
    ...
    [self.selfDescribeView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo( self.resumeEducationView.mas_bottom );
        make.left.equalTo( _backgroundScrollView.mas_left );
        make.width.equalTo( @( kScreenWidth ) );
        make.height.equalTo( @( [CPResumeEditReformer selfDescribeTotalHeight:self.viewModel.resumeNameModel] ) );
    }];
...
- (void)resetOtherFrame
{
    ...
    CGFloat practiceH = 0;
    CGFloat trainH = 0;
    workExperienceH = [CPResumeEditReformer workExperienceTotalHeight:self.viewModel.resumeNameModel];
    ...
    practiceH = [CPResumeEditReformer practiceTotalHeight:self.viewModel.resumeNameModel];
    trainH = [CPResumeEditReformer trainTotalHeight:self.viewModel.resumeNameModel];
    switch ( tempBlockType )
    {
        case CPResumeEditWorkExperienceBlock:
        {
            [self.resumeWorkExperienceView mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo( self.moreButton.mas_bottom );
                make.left.equalTo( _backgroundScrollView.mas_left );
                make.width.equalTo( @( kScreenWidth ) );
                make.height.equalTo( @( workExperienceH ) );
            }];
            break;
        }
        ...
        case CPResumeEditPracticeBlock:
        {
            [self.practiceView mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo( self.projectExperienceView.mas_bottom );
                make.left.equalTo( _backgroundScrollView.mas_left );
                make.width.equalTo( @( kScreenWidth ) );
                make.height.equalTo( @( practiceH ) );
            }];
            break;
        }
        case CPResumeEditTrainBlock:
        {
            [self.trainExperienceView mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo( self.practiceView.mas_bottom );
                make.left.equalTo( _backgroundScrollView.mas_left );
                make.width.equalTo( @( kScreenWidth ) );
                make.height.equalTo( @( trainH ) );
            }];
            break;
        }
        case CPResumeEditAboutSkillBlock:
        {
            [self.aboutSkillView mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo( self.trainExperienceView.mas_bottom );
                make.left.equalTo( _backgroundScrollView.mas_left );
                make.width.equalTo( @( kScreenWidth ) );
...
- (void)respondMoreButton
{
    ...
    if ( self.moreButton.isSelected )
    {
        projectExperienceH = [CPResumeEditReformer workExperienceTotalHeight:self.viewModel.resumeNameModel];
        selfDescribeH = [CPResumeEditReformer projectExperienceTotalHeight:self.viewModel.resumeNameModel];
        aboutSkillH = [CPResumeEditReformer aboutSkillHeight:self.viewModel.resumeNameModel];
        attachH = [CPResumeEditReformer resumeEditAdditionHeight:self.viewModel.resumeNameModel];
        practiceH = [CPResumeEditReformer practiceTotalHeight:self.viewModel.resumeNameModel];
        trainH = [CPResumeEditReformer trainTotalHeight:self.viewModel.resumeNameModel];
...
- (UIView *)firstHeaderView
{
    if ( !_firstHeaderView )
    {
        _firstHeaderView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, ( 140 + 144 + 144 ) / CP_GLOBALSCALE + kScreenHeight)];
...
- (UIScrollView *)backgroundScrollView
{
    if ( !_backgroundScrollView )
    {
        ...
        [self.moreButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo( self.selfDescribeView.mas_bottom ).offset( 40 / CP_GLOBALSCALE );
            make.height.equalTo( @( (144) / CP_GLOBALSCALE ) );
            make.width.equalTo( @( kScreenWidth - 40 / CP_GLOBALSCALE * 2 ) );
            make.centerX.equalTo( self.selfDescribeView.mas_centerX );
        }];
        ...
        [_backgroundScrollView addSubview:self.practiceView];
        ...
        [_backgroundScrollView addSubview:self.trainExperienceView];
        [self.trainExperienceView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo( self.practiceView.mas_bottom );
            make.left.equalTo( _backgroundScrollView.mas_left );
            make.width.equalTo( @( kScreenWidth ) );
            make.height.equalTo( @( 0 ) );
...
- (CPResumeWorkExperienceView *)resumeWorkExperienceView
{
    if ( !_resumeWorkExperienceView )
    {
        ...
        [_resumeWorkExperienceView.editResumeButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            ...
            [CPResumeEditReformer saveResumeEditBlock:CPResumeEditWorkExperienceBlock];
        }];
        [_resumeWorkExperienceView.addMoreButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            ...
            {
                ResumeAddJobVC *vc = [[ResumeAddJobVC alloc] initWithResumeId:self.viewModel.resumeId];
                [self.navigationController pushViewController:vc animated:YES];
            }
        }];
    }
...
- (CPResumeEducationView *)resumeEducationView
{
    if ( !_resumeEducationView )
    {
        ...
        [_resumeEducationView.addMoreButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            ...
            {
                EducationListVC *vc = [[EducationListVC alloc] initWithResumeId:self.viewModel.resumeId isSocial:YES];
                [self.navigationController pushViewController:vc animated:YES];
            }
        }];
    }
...
- (CPResumePracticeView *)practiceView
{
    if ( !_practiceView )
    {
        _practiceView = [[CPResumePracticeView alloc] initWithFrame:self.view.bounds];
        [_practiceView.editResumeButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            [CPResumeEditReformer saveResumeEditBlock:CPResumeEditPracticeBlock];
            AddPracticeVC *vc = [[AddPracticeVC alloc] initWithModel:self.viewModel.resumeNameModel];
            [self.navigationController pushViewController:vc animated:YES];
        }];
        [_practiceView.addMoreButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            [CPResumeEditReformer saveResumeEditBlock:CPResumeEditPracticeBlock];
            AddPracticeVC *vc = [[AddPracticeVC alloc] initWithModel:self.viewModel.resumeNameModel];
            [self.navigationController pushViewController:vc animated:YES];
        }];
...
- (CPResumeTrainExperienceView *)trainExperienceView
{
    if ( !_trainExperienceView )
    {
        _trainExperienceView = [[CPResumeTrainExperienceView alloc] initWithFrame:self.view.bounds];
        [_trainExperienceView.editResumeButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            [CPResumeEditReformer saveResumeEditBlock:CPResumeEditTrainBlock];
            AddTrainVC *vc = [[AddTrainVC alloc] initWithModel:self.viewModel.resumeNameModel];
            [self.navigationController pushViewController:vc animated:YES];
        }];
        [_trainExperienceView.addMoreButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            [CPResumeEditReformer saveResumeEditBlock:CPResumeEditTrainBlock];
            TrainListVC *vc = [[TrainListVC alloc] initWithResumeId:self.viewModel.resumeId];
            [self.navigationController pushViewController:vc animated:YES];
        }];
    }
    return _trainExperienceView;
}
...
- (UIView *)rightMenuView
{
    if ( !_rightMenuView )
    {
        ...
        [expectWorkBtn handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            [weakSelf.backgroundScrollView setContentOffset:CGPointMake(0, weakSelf.resumeExpectWorkView.viewY) animated:YES];
            [weakSelf hideRightMenu];
        ...
        [workExperienceBtn configWithTitel:@"教育经历" imageName:@"resume_education"];
        ...
        [educationExperienceBtn configWithTitel:@"自我描述" imageName:@"resume_education"];
        ...
        [educationExperienceBtn handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            if ( weakSelf.selfDescribeView.viewY < kScreenHeight )
                return;
            [weakSelf.backgroundScrollView setContentOffset:CGPointMake(0, self.selfDescribeView.viewY) animated:YES];
            [weakSelf hideRightMenu];
//            [MobClick event:@"slid_to_education"];
        }];
    }
...
- (UIButton *)deliveryButton
{
    if ( !_deliveryButton )
    {
        ...
        [_deliveryButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            ...
            if ( [weakSelf.schoolResumeEditDelegate respondsToSelector:@selector(schoolResumeEdit:deliveryResume:)] )
            {
                [weakSelf.schoolResumeEditDelegate schoolResumeEdit:weakSelf deliveryResume:weakSelf.viewModel.resumeNameModel];
            }
        }];
    }
