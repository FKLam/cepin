...
#import "CPCommon.h"
@interface CPBindExistAccountVM ()
...
@end
@implementation CPBindExistAccountVM
- (instancetype)init
{
    if (self = [super init]) {
    }
...
- (void)bindUser
{
    ...
    NSString *usrID = [[MemoryCacheData shareInstance] userId];
    NSString *usrToken = [[MemoryCacheData shareInstance] userTokenId];
    // inviteCode 渠道号
    RACSignal *signal = [[RTNetworking shareInstance] bindExistAccountWithAccount:self.account passWord:self.password serId:usrID tokenId:usrToken bindType:@"2"];
    ...
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        {
            [self showLongMessage:[dic resultErrorMessage]];
...
{
    self.existAccountTipsView = [self longMessageTipsView:message];
    [[UIApplication sharedApplication].keyWindow addSubview:self.existAccountTipsView];
}
...
- (UIView *)shortMessageTipsView:(NSString *)message
{
    ...
    {
        _existAccountTipsView = [[UIView alloc] initWithFrame:self.setViewController.view.bounds];
...
- (UIView *)longMessageTipsView:(NSString *)message
{
    if ( !_existAccountTipsView )
    {
        _existAccountTipsView = [[UIView alloc] initWithFrame:self.setViewController.view.bounds];
