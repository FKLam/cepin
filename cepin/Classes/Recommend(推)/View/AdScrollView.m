...
@property (retain,nonatomic,strong) UIButton * leftButton;
@property (retain,nonatomic,strong) UIButton * centerButton;
@property (retain,nonatomic,strong) UIButton * rightButton;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _leftButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, UISCREENWIDTH, UISCREENHEIGHT)];
        [self addSubview:_leftButton];
        _centerButton= [[UIButton alloc]initWithFrame:CGRectMake(UISCREENWIDTH, 0, UISCREENWIDTH, UISCREENHEIGHT)];
        [self addSubview:_centerButton];
        _rightButton = [[UIButton alloc]initWithFrame:CGRectMake(UISCREENWIDTH*2, 0, UISCREENWIDTH, UISCREENHEIGHT)];
        [self addSubview:_rightButton];
        UITapGestureRecognizer *Tap =[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(bannerImagePressed:)];
        ...
        [_rightButton addTarget:self action:@selector(bannerButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        [_centerButton addTarget:self action:@selector(bannerButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
        [_rightButton addTarget:self action:@selector(bannerButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)bannerButtonPressed:(UIButton *)sender
{
//    NSLog(@"index==%ld",_pageControl.currentPage );
    [self.bannerDelegate didPushWith:_pageControl.currentPage];
}
...
- (void)setImageNameArray:(NSArray *)imageNameArray
{
    ...
    BannerModel *model = [BannerModel beanFromDictionary:imageNameArray[0]];
    NSString *urlStr = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)model.ImgUrl, NULL, NULL, kCFStringEncodingUTF8));
    [_leftImageView sd_setImageWithURL:[NSURL URLWithString:urlStr]
              placeholderImage:[UIImage imageNamed:@"ad_loading_img"]];
    
    BannerModel *model1 = [BannerModel beanFromDictionary:imageNameArray[1]];
    urlStr = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)model1.ImgUrl, NULL, NULL, kCFStringEncodingUTF8));
    [_centerImageView sd_setImageWithURL:[NSURL URLWithString:urlStr]
                   placeholderImage:[UIImage imageNamed:@"ad_loading_img"]];
    
    BannerModel *model2 = [BannerModel beanFromDictionary:imageNameArray[2]];
    urlStr = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)model2.ImgUrl, NULL, NULL, kCFStringEncodingUTF8));
    [_rightImageView sd_setImageWithURL:[NSURL URLWithString:urlStr]
                     placeholderImage:[UIImage imageNamed:@"ad_loading_img"]];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    BannerModel *model = [BannerModel beanFromDictionary:_imageNameArray[(currentImage - 1) % _imageNameArray.count]];
    NSString *urlStr = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)model.ImgUrl, NULL, NULL, kCFStringEncodingUTF8));
    [_leftImageView sd_setImageWithURL:[NSURL URLWithString:urlStr]
                   placeholderImage:[UIImage imageNamed:@"ad_loading_img"]];
    ...
    BannerModel *model1 = [BannerModel beanFromDictionary:_imageNameArray[currentImage % _imageNameArray.count]];
    urlStr = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)model1.ImgUrl, NULL, NULL, kCFStringEncodingUTF8));
    [_centerImageView sd_setImageWithURL:[NSURL URLWithString:urlStr]
                     placeholderImage:[UIImage imageNamed:@"ad_loading_img"]];
    ...
    BannerModel *model2 = [BannerModel beanFromDictionary:_imageNameArray[(currentImage + 1) % _imageNameArray.count]];
    urlStr = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)model2.ImgUrl, NULL, NULL, kCFStringEncodingUTF8));
    [_rightImageView sd_setImageWithURL:[NSURL URLWithString:urlStr]
                    placeholderImage:[UIImage imageNamed:@"ad_loading_img"]];
