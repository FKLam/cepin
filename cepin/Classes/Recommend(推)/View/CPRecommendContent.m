...
@implementation CPRecommendContent {
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _contentTextFont = [UIFont fontWithName:@"HelveticaNeue-Light" size:17];
        _lineSpace = 5;
    }
    return self;
}
    ...
    for(NSString* key in definition) {
        ...
        for(NSTextCheckingResult* match in matches) {
            ...
            if(!highlightColors||!(textColor=([highlightColors objectForKey:key])))
                textColor = self.contentTextColor;
...
- (void)setText:(NSString *)text{
    ...
    if ([text isEqualToString:_contentText]) {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        UIColor* textColor = self.contentTextColor;
        
        //Set line height, font, color and break mode
        CGFloat minimumLineHeight = self.contentTextFont.pointSize,maximumLineHeight = minimumLineHeight, linespace = self.lineSpace;
        CTFontRef font = CTFontCreateWithName((__bridge CFStringRef)self.contentTextFont.fontName, self.contentTextFont.pointSize,NULL);
        CTLineBreakMode lineBreakMode = kCTLineBreakByWordWrapping;
        CTTextAlignment alignment = CTTextAlignmentFromUITextAlignment(self.contentTextAlignment);
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        if (lineIndex == numberOfLines - 1 && truncateLastLine) {
            ...
            if (!(lastLineRange.length == 0 && lastLineRange.location == 0) && lastLineRange.location + lastLineRange.length < textRange.location + textRange.length) {
                ...
                y = lineOrigin.y - descent - self.contentTextFont.descender;
        ...
        if ((!highlighting&&self.superview!=nil)) {
            ...
            for (int j = 0; j < CFArrayGetCount(runs); j++) {
                ...
                if (!CGColorEqualToColor((__bridge CGColorRef)([attributes valueForKey:@"CTForegroundColor"]), self.contentTextColor.CGColor)
                    && framesDict!=nil) {
...
- (BOOL)touchPoint:(CGPoint)point{
    for (NSString *key in framesDict.allKeys) {
        ...
        if (CGRectContainsPoint(frame, point)) {
            ...
            NSArray* matches = [AccountRegularExpression() matchesInString:self.contentText options:0 range:NSMakeRange(0, self.contentText.length)];
            for(NSTextCheckingResult* match in matches) {
            ...
            matches = [TopicRegularExpression() matchesInString:self.contentText options:0 range:NSMakeRange(0, self.contentText.length)];
            for(NSTextCheckingResult* match in matches) {
                if (range.location!=-1 && range.location>=match.range.location && range.length+range.location<=match.range.length+match.range.location) {
                    return YES;
                }
