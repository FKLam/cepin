...
@end
@implementation ExperListVM
- (instancetype)initWithResumeModel:(ResumeNameModel *)model
{
    self = [super init];
    if (self) {
        self.pracDatas = [NSMutableArray new];
        self.pracModel = [PracticeListDataModel new];
...
- (void)getPracticeList
{
   self.load = [TBLoading new];
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumePracticeListWithResumeId:self.resumeId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.pracDatas = [dic resultObject];
...
- (void)deletePracticeListWith:(NSString *)practiceId
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumePracticeListWithResumeId:self.resumeId Id:practiceId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
        if (load)
