...
@end
@implementation EditAbiltyVM
- (instancetype)initWithWork:(SkillDataModel*)model
{
    self = [super init];
    if (self) {
        self.skillData = [SkillDataModel new];
        self.skillData = model;
        self.skillId = model.Id;
...
- (BOOL)request
{
    if (!self.skillData.Name ||[self.skillData.Name isEqualToString:@""])
    {
        [self showShordMessage:@"请输入技能名称"];
        return NO;
    }
    
    if (!self.skillData.MasteryLevel || [self.skillData.MasteryLevel isEqualToString:@""])
    {
        [self showShordMessage:@"请选择掌握程度"];
        return NO;
    }
    return YES;
...
- (void)saveSkill
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]saveThridSkillUserId:userId tokenId:TokenId Name:self.skillData.Name MasteryLevel:self.skillData.MasteryLevel ResumeId:self.resumeId Id:self.skillId];
    @weakify(self);
