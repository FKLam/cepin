...
#import "CPCommon.h"
@interface ResumeEditJobExperienceVM ()
...
@end
@implementation ResumeEditJobExperienceVM
...
- (BOOL)request
{
    ...
    if ( !self.workData.JobFunctionKey || [self.workData.JobFunctionKey length] == 0 )
    {
...
- (void)getWorkInfo
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumeWorkListWithResumeId:self.resumeId Id:self.workId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.workData = [WorkListDateModel beanFromDictionary:[dic resultObject]];
...
- (void)saveWork
{
    ...
    if ( self.resumeType.intValue == 2 )
    {
        signal = [[RTNetworking shareInstance] saveThridWorkStartDate:self.workData.StartDate userId:userId tokenId:TokenId EndDate:self.workData.EndDate Nature:self.workData.Nature Size:self.workData.Size Industry:self.workData.Industry JobCity:self.workData.JobCity JobFunction:self.workData.JobFunction Content:self.workData.Content Salary:self.workData.Salary CompanyRanking:self.workData.CompanyRanking ResumeId:self.workData.ResumeId Id:self.workData.Id NatureKey:self.workData.NatureKey IndustryKey:self.workData.IndustryKey JobCityKey:self.workData.JobCityKey JobFunctionKey:self.workData.JobFunctionKey SizeKey:self.workData.SizeKey Company:self.workData.Company IsAbroad:[NSString stringWithFormat:@"%@", isAbroad] AttestorName:self.workData.AttestorName AttestorRelation:self.workData.AttestorRelation AttestorPosition:self.workData.AttestorPosition AttestorCompany:self.workData.AttestorCompany AttestorPhone:self.workData.AttestorPhone];
    }
    else
    {
        signal = [[RTNetworking shareInstance] saveThridWorkStartDate:self.workData.StartDate userId:userId tokenId:TokenId EndDate:self.workData.EndDate Nature:self.workData.Nature Size:self.workData.Size Industry:self.workData.Industry JobCity:self.workData.JobCity JobFunction:self.workData.JobFunction Content:self.workData.Content Salary:self.workData.Salary CompanyRanking:self.workData.CompanyRanking ResumeId:self.workData.ResumeId Id:self.workData.Id NatureKey:self.workData.NatureKey IndustryKey:self.workData.IndustryKey JobCityKey:self.workData.JobCityKey JobFunctionKey:[NSString stringWithFormat:@"%@", self.workData.JobFunctionKey] SizeKey:self.workData.SizeKey Company:self.workData.Company IsAbroad:[NSString stringWithFormat:@"%@", isAbroad]];
    }
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
