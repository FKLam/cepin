...
#import "EditGanBuVM.h"
#import "RTNetworking+Resume.h"
#import "TBtextUnit.h"
#import "NSDate-Utilities.h"
#import "CPPositionDetailDescribeLabel.h"
#import "CPCommon.h"
@interface EditGanBuVM ()
...
@end
@implementation EditGanBuVM
- (instancetype)initWithWork:(StudentLeadersDataModel *)model
{
    self = [super init];
    if (self) {

        self.studData = [StudentLeadersDataModel new];
        self.studData = model;
...
- (BOOL)request
{
    ...
    if (!self.studData.StartDate || [self.studData.StartDate isEqualToString:@""])
    {
        [self showShordMessage:@"请选择开始时间"];
        return NO;
    }
    if (!self.studData.EndDate || [self.studData.EndDate isEqualToString:@""])
    {
        NSDate *date = [NSDate date];
        NSString *strYear = [date stringyyyyFromDate];
        NSString *strMonth = [date stringMMFromDate];
        self.studData.EndDate = [NSString stringWithFormat:@"%@-%@-1",strYear,strMonth];
        NSArray *beginArray = [self.studData.StartDate componentsSeparatedByString:@"-"];
        NSArray *endArray = [self.studData.EndDate componentsSeparatedByString:@"-"];
    ...
    if (!self.studData.Name || [self.studData.Name isEqualToString:@""])
    {
        [self showShordMessage:@"请填写职务名称"];
        return NO;
    }
    if (!self.studData.Level || [self.studData.Level isEqualToString:@""])
    {
        [self showShordMessage:@"请选择类型"];
        return NO;
    }
    return YES;
}
...
- (void)saveStudent
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]saveThridStudentStartDate:self.studData.StartDate EndDate:self.studData.EndDate userId:userId tokenId:TokenId Level:self.studData.Level Name:self.studData.Name ResumeId:self.resumeId Id:self.studentId type:@"1"];
    @weakify(self);
