...
@end
@implementation LanguageVM
- (instancetype)initWithResumeModel:(ResumeNameModel *)model
{
    self = [super init];
    if (self) {
        self.langDatas = [NSMutableArray new];
        self.langModel = [LanguageDataModel new];
...
- (void)getLanguage
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumeLanguageListWithResumeId:self.resumeId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.langDatas = [dic resultObject];
...
- (void)dellteLanguage:(NSString *)languageId
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumeLanguageListWithResumeId:self.resumeId Id:languageId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
        if (load)
