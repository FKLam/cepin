...
@end
@implementation AddEducationVM
- (instancetype)initWithResumeModel:(ResumeNameModel *)model
{
    self = [super init];
    if (self) {
        self.eduDatas = [NSMutableArray new];
        self.eduModel = [EducationListDateModel new];
...
- (void)getEducationList
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumeEducationListWithResumeId:self.resumeId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.eduDatas = [dic resultObject];
...
- (void)deleteEduListWith:(EducationListDateModel*)model
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumeEducationListWithResumeId:self.resumeId eduId:model.Id userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
        if (load)
        ...
        if ([dic resultSucess])
        {
            [self.eduDatas removeObject:model];
