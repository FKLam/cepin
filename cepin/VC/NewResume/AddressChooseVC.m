...
#import "CPCommon.h"
@interface AddressChooseVC ()<CPResumeEditFirstAddressCellDelegate, AddressChooseVMDelegate, UITextFieldDelegate, UIGestureRecognizerDelegate>
@property (nonatomic, retain) AddressChooseVM *viewModel;
@property (nonatomic, assign) BOOL isjg;
@property (nonatomic, strong) UIView *searchTopView;
@property (nonatomic, strong) UIView *topView;
@property (nonatomic, strong) CPResumeInfoAddressSearchTextField *searchCityTextField;
...
@end
@implementation AddressChooseVC
-(instancetype)initWithModel:(ResumeNameModel*)model isJG:(BOOL)isJG
{
    if (self = [super init])
    {
        self.isjg = isJG;
        self.viewModel = [[AddressChooseVM alloc] initWithSendModel:model];
        self.viewModel.addressChooseDelegate = self;
        if (isJG) {
            self.viewModel.selectedCity = [Region searchRegionWithAddressString:model.NativeCity];
        }else{
            self.viewModel.selectedCity = [Region searchRegionWithAddressString:model.Region];
        }
...
- (void)textFieldTextDidChangeOneCI:(UITextField *)textField
{
    ...
    NSArray *searchResultArray = [CPResumeEditInformationReformer searchMatchAddressWithMatchString:textField.text originArray:[self.allCityArrayM copy]];
...
#pragma mark - AddressChooseVMDelegate
...
- (void)clickedHotCityButton:(UIButton *)sender
{
    ...
    if(nil == self.viewModel.selectedCity){
        self.viewModel.selectedCity = [NSMutableArray array];
    }
    [self.viewModel.selectedCity addObject:region];
    Region *it = self.viewModel.selectedCity[0];
    if(self.isjg){
...
{
    [self.view endEditing:YES];
    if ( scrollView == self.searchCityTableView && !self.searchCityTableView.isHidden && 0 == [self.searchCityResultArrayM count] )
        [self.searchCityTableView setHidden:YES];
}
...
//            intNum = 7;
        ...
        [self.localButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            ...
            [weakSelf.viewModel.selectedCity addObject:region];
            Region *it = weakSelf.viewModel.selectedCity[0];
            if(weakSelf.isjg){
                weakSelf.model.NativeCity = it.RegionName;
                weakSelf.model.NativeCityKey = it.PathCode;
            }
            else
            {
                //统计热门城市
                [MobClick event:@"jzd_hot_region"];
                weakSelf.model.Region = it.RegionName;
                weakSelf.model.RegionCode = it.PathCode;
            }
...
- (CPResumeInfoAddressSearchTextField *)searchCityTextField
{
    if ( !_searchCityTextField )
    {
        CPResumeInfoAddressSearchTextField *text = [[CPResumeInfoAddressSearchTextField alloc] init];
        ...
        text.background = [[UIImage imageWithColor:[UIColor colorWithHexString:@"efeff4"] cornerRadius:0.0] resizableImageWithCapInsets:UIEdgeInsetsMake(8, 8, 8, 8)];
        _searchCityTextField = text;
        [_searchCityTextField addTarget:self action:@selector(textFieldTextDidChangeOneCI:) forControlEvents:UIControlEventEditingChanged];
    }
...
- (NSMutableArray *)searchCityResultArrayM
{
    if ( !_searchCityResultArrayM )
    {
        _searchCityResultArrayM = [NSMutableArray array];
    }
    return _searchCityResultArrayM;
}
@end
