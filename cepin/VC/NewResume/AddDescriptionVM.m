...
@end
@implementation AddDescriptionVM
...
- (void)saveInfo
{
    if ( self.resumeModel.UserRemark.length > 500 ) {
        [self showShordMessage:@"自我描述不能超过500字"];
        return;
    }
    ...
    RACSignal *signal = [[RTNetworking shareInstance] saveThridAdditionInfoUserId:userId tokenId:TokenId ResumeId:self.resumeId UserRemark:self.resumeModel.UserRemark];
    @weakify(self);
...
- (void)saveOneWord
{
    if (self.resumeModel.UserRemark.length > 100) {
        [self showShordMessage:@"一句话优势不能超过100字"];
        return;
    }
    TBLoading *load = [TBLoading new];
    [load start];
