...
#import "CPCommon.h"
@interface ResumeJobExperienceVM ()
...
@end
@implementation ResumeJobExperienceVM
- (instancetype)initWithResumeModel:(ResumeNameModel *)model
{
    self = [super init];
    if (self) {
        self.jobDatas = [NSMutableArray new];
        self.workModel = [WorkListDateModel new];
...
- (void)getWorkList
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumeWorkListWithResumeId:self.resumeId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.jobDatas = [dic resultObject];
...
- (void)deleteWorkListWith:(NSString *)workId
{
    ...
    NSString *userId = [MemoryCacheData shareInstance].userLoginData.UserId;
    NSString *TokenId = [MemoryCacheData shareInstance].userLoginData.TokenId;
    RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumeWorkListWithResumeId:self.resumeId workId:workId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
        if (self.load)
