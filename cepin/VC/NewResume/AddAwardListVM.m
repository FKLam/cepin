...
- (BOOL)request
{
    if ([self.awardData.Level isEqualToString:@""] || !self.awardData.Level )
    {
...
- (void)addAward
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]addThridAwardStartDate:self.awardData.StartDate userId:userId tokenId:TokenId Level:self.awardData.Level Name:self.awardData.Name ResumeId:self.resumeId type:@"3"];
    @weakify(self);
