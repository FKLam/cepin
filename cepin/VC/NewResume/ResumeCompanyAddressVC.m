...
#import "CPPinyin.h"
#import "CPCommon.h"
@interface ResumeCompanyAddressVC ()<ResumeCompanyAddressVMDelegate, CPResumeEditFirstAddressCellDelegate,UITextFieldDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, retain) ResumeCompanyAddressVM *viewModel;
@end
@implementation ResumeCompanyAddressVC
-(instancetype)initWithWorkModel:(WorkListDateModel *)model
{
    if (self = [super init])
    {
        self.viewModel = [[ResumeCompanyAddressVM alloc] initWithWorkModel:model];
...
- (void)clickedHotCityButton:(UIButton *)sender
{
    ...
    self.model.JobCity = it.RegionName;
    self.model.JobCityKey = it.PathCode;
...
{
    if ( self.searchCityTableView.isHidden )
        [self.searchCityTableView setHidden:NO];
    return YES;
}
...
//            intNum = 7;
        ...
        [self.localButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            ...
            if(nil != weakSelf.viewModel.selectedCity && weakSelf.viewModel.selectedCity.count>0){
