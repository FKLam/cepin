...
@implementation FullResumeMenuView
-(id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        self.titlesAndImages = @[@"简历预览",@"设为默认",@"简历删除"];
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(10, self.viewHeight, 300, (IS_IPHONE_5?40:48)*3) style:UITableViewStylePlain];
...
-(void)startApper
{
    ...
    rect.origin.y = self.viewHeight-(IS_IPHONE_5?40:48)*3-5;
    self.hidden = NO;
...
//    cell.imageIcon.image = [UIImage imageNamed:self.titlesAndImages[index][1]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self startDisapper];
    if (self.delegate && [self.delegate respondsToSelector:@selector(didFullResumeMenuTouch:)])
    {
        [self.delegate didFullResumeMenuTouch:indexPath.row];
    }
}
