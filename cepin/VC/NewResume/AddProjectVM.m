...
@end
@implementation AddProjectVM
- (instancetype)initWithResumeModel:(ResumeNameModel*)model
{
    self = [super init];
    if (self) {
        self.projectDatas = [NSMutableArray new];
        self.projedctModel = [ProjectListDataModel new];
...
- (void)getProjectList
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumeProjectListWithResumeId:self.resumeId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.projectDatas = [dic resultObject];
...
- (void)deleteProjectListWith:(NSString*)ProjectId
{
    ...
    NSString *userId = [MemoryCacheData shareInstance].userLoginData.UserId;
    NSString *TokenId = [MemoryCacheData shareInstance].userLoginData.TokenId;
    RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumeProjectListWithResumeId:self.resumeId Id:ProjectId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
        if (load)
