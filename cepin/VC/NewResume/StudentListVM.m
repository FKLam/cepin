...
@end
@implementation StudentListVM
- (instancetype)initWithResumeModel:(ResumeNameModel *)model
{
    self = [super init];
    if (self) {
        self.studDatas = [NSMutableArray new];
        self.studModel = [StudentLeadersDataModel new];
...
- (void)getStudentList
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumeAwardListWithResumeId:self.resumeId userId:userId tokenId:TokenId type:@"1"];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.studDatas = [dic resultObject];
...
- (void)deleteStudentListWith:(NSString *)studentId
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumeAwardListWithResumeId:self.resumeId Id:studentId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
        if (load)
