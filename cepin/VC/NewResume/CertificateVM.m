...
@end
@implementation CertificateVM
- (instancetype)initWithResumeModel:(ResumeNameModel *)model
{
    self = [super init];
    if (self) {
        self.credentialDatas = [NSMutableArray new];
        self.credentialModel = [CredentialListDataModel new];
...
- (void)getcredentialList
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumeCredentialListWithResumeId:self.resumeId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.credentialDatas = [dic resultObject];
...
- (void)deletecredentialListWith:(NSString *)credentialId
{
    ...
    NSString *userId = [MemoryCacheData shareInstance].userLoginData.UserId;
    NSString *TokenId = [MemoryCacheData shareInstance].userLoginData.TokenId;
    RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumeCredentialListWithResumeId:self.resumeId Id:credentialId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
        if (load)
