...
@end
@implementation EditCertificateVM
- (instancetype)initWithWork:(CredentialListDataModel*)model
{
    self = [super init];
    if (self) {
        self.credData = [CredentialListDataModel new];
        self.credData = model;
        self.credId = model.Id;
...
- (BOOL)request
{
    if (!self.credData.Name || [self.credData.Name isEqualToString:@""])
    {
        [self showShordMessage:@"请选择证书名称"];
        return NO;
    }
    if (!self.credData.BaseCodeKey)
    {
        self.credData.BaseCodeKey = @"";
    }
    if (!self.credData.Date || [self.credData.Date isEqualToString:@""])
    {
...
- (void)saveCred
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance] saveThridCredentialDate:self.credData.Date userId:userId tokenId:TokenId Name:self.credData.Name BaseCodeKey:self.credData.BaseCodeKey ResumeId:self.resumeId Id:self.credId Type:@"2"];
    @weakify(self);
