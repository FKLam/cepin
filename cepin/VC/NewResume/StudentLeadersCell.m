...
@implementation StudentLeadersCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.name = [[UILabel alloc]initWithFrame:CGRectMake(self.time.viewX, self.time.viewHeight + 10, self.viewWidth- 40, hight)];
        ...
        self.level = [[UILabel alloc]initWithFrame:CGRectMake(self.name.viewX, self.name.viewY + self.name.viewHeight + 10, self.viewWidth - 40, hight)];
...
{
    StudentLeadersDataModel *StuModel = model;
    ...
    self.name.text = [NSString stringWithFormat:@"%@  |  %@", StuModel.Name, StuModel.Level];
}
