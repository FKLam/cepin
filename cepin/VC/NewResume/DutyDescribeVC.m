...
#import "CPTipsView.h"
#import "CPCommon.h"
@interface DutyDescribeVC ()<UITextViewDelegate, CPTipsViewDelegate>
...
- (void)viewDidLoad
{
    ...
    [[self addNavicationObjectWithType:NavcationBarObjectTypeConfirm] handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
        ...
        if (self.describeText.text.length > 60) {
            [self shomMessageTips:@"你的职责不能超过60字符"];
            return ;
        }
        if ( [self.duty length] > 0 )
        {
            if ( ![self checkWithString:self.duty] )
            {
                [self shomMessageTips:@"请输入你的职责"];
                return;
            }
        }
        else
    ...
    self.describeText.text = self.duty;
    ...
    self.titleLabel.text = @"请描述你在项目中负责的具体工作";
    ...
    if (!self.model.Description || [self.model.Description isEqualToString:@""]) {
    ...
    [[self.describeText rac_textSignal] subscribeNext:^(NSString *text) {
        ...
        if (text)
        {
            NSInteger len = 60 - text.length;
            if( len < 0 ){
                self.describeText.text = [self.describeText.text substringToIndex:60];
                NSString *des = [NSString stringWithFormat:@"60/60"];
                NSMutableAttributedString *str = [[NSMutableAttributedString alloc]initWithString:des];
                [str addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"288add"] range:NSMakeRange(0, 3)];
                countLabel.attributedText = str;
                self.duty = [text substringToIndex:60];
                return ;
            }
            NSString *des = [NSString stringWithFormat:@"%ld/60",(long)text.length];
            NSString *lenStr = [NSString stringWithFormat:@"%ld",text.length];
