...
@end
@implementation EditTrainVM
- (instancetype)initWithWork:(TrainingDataModel *)model
{
    self = [super init];
    if (self) {
        self.trainData = [TrainingDataModel new];
        self.trainData = model;
        self.trainId = model.Id;
...
- (BOOL)request
{
    ...
    if (!self.trainData.StartDate || [self.trainData.StartDate isEqualToString:@""])
    {
        [self showShordMessage:@"请选择开始时间"];
        return NO;
    }
    if (!self.trainData.EndDate || [self.trainData.EndDate isEqualToString:@""])
    {
        NSDate *date = [NSDate date];
        NSString *strYear = [date stringyyyyFromDate];
        NSString *strMonth = [date stringMMFromDate];
        self.trainData.EndDate = [NSString stringWithFormat:@"%@-%@-1",strYear,strMonth];
        NSArray *beginArray = [self.trainData.StartDate componentsSeparatedByString:@"-"];
        NSArray *endArray = [self.trainData.EndDate componentsSeparatedByString:@"-"];
    ...
    if (!self.trainData.Name || [self.trainData.Name isEqualToString:@""])
    {
        [self showShordMessage:@"请填写培训课程"];
        return NO;
    }
    if (!self.trainData.Organization || [self.trainData.Organization isEqualToString:@""])
    {
        [self showShordMessage:@"请填写培训结构"];
        return NO;
    }
    if (!self.trainData.Content || [self.trainData.Content isEqualToString:@""])
    {
        [self showShordMessage:@"请填写培训内容"];
        return NO;
    }
    return YES;
}
...
- (void)saveTrain
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]saveThridTrainStartDate:self.trainData.StartDate userId:userId tokenId:TokenId EndDate:self.trainData.EndDate Organization:self.trainData.Organization Name:self.trainData.Name Content:self.trainData.Content ResumeId:self.resumeId Id:self.trainId];
    @weakify(self);
