...
#import "CPCommon.h"
@interface EditGanBuVC ()<UITextFieldDelegate, ResumeThridTimeViewDelegate, UIGestureRecognizerDelegate>
@property(nonatomic,retain)ResumeThridTimeView *timeView;
@property(nonatomic,strong)EditGanBuVM *viewModel;
@property(nonatomic,strong)UITextField *textField;
@property (nonatomic, strong) UIView *secondFooterView;
@property (nonatomic, strong) UIButton *deleteButton;
@end
@implementation EditGanBuVC
- (instancetype)initWithModel:(StudentLeadersDataModel *)model
{
    self = [super init];
    if (self) {
        self.viewModel = [[EditGanBuVM alloc]initWithWork:model];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        case 0:
        {
            [cell configCellLeftString:@"开始时间" placeholder:@"请选择"];
            [cell.inputTextField setTag:indexPath.row];
        ...
        case 1:
        {
            [cell configCellLeftString:@"结束时间" placeholder:@"请选择"];
            [cell.inputTextField setTag:indexPath.row];
        ...
        case 3:
        {
            [cell configCellLeftString:@"类型" placeholder:@"请选择"];
            [cell.inputTextField setTag:indexPath.row];
...
- (UIButton *)deleteButton
{
    if ( !_deleteButton )
    {
        ...
        [_deleteButton handleControlEvent:UIControlEventTouchUpInside withBlock:^(id sender) {
            ...
            RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumeAwardListWithResumeId:self.viewModel.resumeId Id:self.viewModel.studentId userId:userId tokenId:TokenId];
            @weakify(self);
