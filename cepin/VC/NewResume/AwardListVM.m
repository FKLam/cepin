...
@end
@implementation AwardListVM
- (instancetype)initWithResumeModel:(ResumeNameModel*)model
{
    self = [super init];
    if (self) {
        self.awardDatas = [NSMutableArray new];
        self.awardModel = [AwardsListDataModel new];
...
- (void)getAwardList
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]getThridResumeAwardListWithResumeId:self.resumeId userId:userId tokenId:TokenId type:@"3"];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        ...
        if ([dic resultSucess])
        {
            self.awardDatas = [dic resultObject];
...
- (void)deleteAwardListWith:(NSString*)awardId
{
    ...
    RACSignal *signal = [[RTNetworking shareInstance]deleteThridResumeAwardListWithResumeId:self.resumeId Id:awardId userId:userId tokenId:TokenId];
    @weakify(self);
    [signal subscribeNext:^(RACTuple *tuple){
        @strongify(self);
        if (load)
