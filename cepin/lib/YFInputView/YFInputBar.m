...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, kScreenHeight - CGRectGetHeight(frame), kScreenWidth, CGRectGetHeight(frame));
...
{
    if (!_textField) {
        _textField = [[UITextField alloc]initWithFrame:CGRectMake(10, 10, kScreenWidth - 80, 30)];
...
-(UIButton *)sendBtn
{
    if (!_sendBtn) {
        ...
        _sendBtn.layer.cornerRadius = 4.f;
        _sendBtn.backgroundColor = UIColorFromRGB(0xfb6e52);
//      [_sendBtn setBackgroundColor:[UIColor whiteColor]];
        [_sendBtn setFrame:CGRectMake(kScreenWidth-10-50, 10, 50, 30)];
...
- (void)keyboardWillShow:(NSNotification*)notification{
    ...
    RTLog(@"%f-%f-%f-%f",_keyboardRect.origin.y,_keyboardRect.size.height,[self getHeighOfWindow]-CGRectGetMaxY(self.frame),CGRectGetMinY(self.frame));
