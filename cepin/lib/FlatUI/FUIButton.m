...
- (CGRect)imageRectForContentRect:(CGRect)contentRect
{
    ...
    rect.size.height = contentRect.size.height * 0.4;
    rect.size.width = contentRect.size.height * 0.4;
    rect.origin.x = contentRect.size.width / 3.0;
    CGRect titleRect = self.titleLabel.frame;
    rect.origin.y = titleRect.origin.y;
    return rect;
}
...
- (void) configureFlatButton {
    ...
    UIImage *highlightedBackgroundImage = [UIImage buttonImageWithColor:self.buttonHighlightedColor
                                                           cornerRadius:self.cornerRadius
