...
- (id)initWithImage:(UIImage *)originalImage cropFrame:(CGRect)cropFrame limitScaleRatio:(NSInteger)limitRatio isShowAll:(BOOL)isShowAll
{
    self = [self initWithImage:originalImage cropFrame:cropFrame limitScaleRatio:limitRatio];
    if (self)
    {
...
- (void)initView
{
    ...
    if ( self.isShowAll )
        self.cropFrame = CGRectMake(0, 0, kScreenWidth, kScreenHeight);
    ...
    if ( self.isShowAll )
        self.ratioView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
...
-(UIImage *)getSubImage{
    ...
    if ( self.isShowAll )
        h = squareFrame.size.height / scaleRatio;
